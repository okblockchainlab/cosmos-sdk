...
func TestCoin(t *testing.T) {
	...
	require.Equal(t, NewDec(5), NewCoin(testDenom1, NewInt(5)).Amount)
}
...
func TestSubCoin(t *testing.T) {
	...
	res := tc.inputOne.Sub(tc.inputTwo)
	require.Equal(t, tc.expected, res.Amount.BigInt().Int64())
}
...
func TestParse(t *testing.T) {
	...
	cases := []struct {
		...
		{"5foo-bar", true, Coins{{"foo-bar", NewDec(5)}}},          // once more, only letters in coin name
	}
...
func TestMarshalJSONCoins(t *testing.T) {
	...
	testCases := []struct {
		...
		{"nil coins", nil, `[]`},
		{"empty coins", Coins{}, `[]`},
		{"non-empty coins", NewCoins(NewInt64Coin("foo", 50)), `[{"denom":"foo","amount":"50.00000000"}]`},
	}
