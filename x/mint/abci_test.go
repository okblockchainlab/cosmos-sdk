...
func TestBeginBlocker(t *testing.T) {
	...
	// this year mint test
	curSupply0 := mapp.BankKeeper.GetSupply(ctx).GetTotal().AmountOf(sdk.DefaultBondDenom)
	curCoin0 := curSupply0
	...
	curSupply1 := mapp.BankKeeper.GetSupply(ctx).GetTotal().AmountOf(sdk.DefaultBondDenom)
	curCoin1 := curSupply1
	minter = mapp.mintKeeper.GetMinterCustom(ctx)
